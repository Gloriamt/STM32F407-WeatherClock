...
static void test_truncated_response_has_no_terminal(void)
{
    ...
    assert(feed(&capture, "\r\nO") == AT_RESPONSE_PENDING);
    assert(feed(&capture, "K") == AT_RESPONSE_PENDING);
}

static void test_invalid_terminals_are_ignored(void)
{
    at_response_capture_t capture;
    char buffer[64];
...
static void test_null_and_zero_capacity_capture(void)
{
    at_response_capture_t capture;
    char buffer[1];

    assert(AtResponse_Push(NULL, (uint8_t)'A') == AT_RESPONSE_PENDING);

    AtResponse_Init(&capture, NULL, 0U);
    assert(feed(&capture, "\r\nOK\r\n") == AT_RESPONSE_OK);
    assert(capture.overflowed);
...
int main(void)
{
    ...
    test_invalid_terminals_are_ignored();
    test_null_and_zero_capacity_capture();
